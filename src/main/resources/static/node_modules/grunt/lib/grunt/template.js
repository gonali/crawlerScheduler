...
// Initialize templates delimiters.
...
// The underscore default templates syntax should be a pretty sane default for
// the config system.
template.addDelimiters('config', '<%', '%>');

// Set Lo-Dash templates delimiters.
...
// Process templates + data with Lo-Dash.
template.process = function(tmpl, options) {
  ...
  var last = tmpl;
  try {
    // As long as tmpl contains templates tags, render it and get the result,
    // otherwise just use the templates string.
    while (tmpl.indexOf(delimiters.opener) >= 0) {
      tmpl = grunt.util._.template(tmpl, data);
      // Abort if templates didn't change - nothing left to process!
      if (tmpl === last) { break; }
      last = tmpl;
    }
  } catch (e) {
    ...
    if (String(e) === 'SyntaxError: Unexpected token ILLEGAL' && /\n|\r/.test(tmpl)) {
      grunt.log.errorlns('A special character was detected in this templates. ' +
        'Inside templates tags, the \\n and \\r special characters must be ' +
        'escaped as \\\\n and \\\\r. (grunt 0.4.0+)');
    }
    // Slightly better error message.
    e.message = 'An error occurred while processing a templates (' + e.message + ').';
